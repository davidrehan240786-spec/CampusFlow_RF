...
function LoginPage() {
  ...
  useEffect(() => {
    const getRandomBlinkInterval = () => Math.random() * 3000 + 2000; // More frequent: 2-5 seconds
  ...
  useEffect(() => {
    const getRandomBlinkInterval = () => Math.random() * 3000 + 2000;

    const scheduleBlink = () => {
      const blinkTimeout = setTimeout(() => {
        setIsBlackBlinking(true);
        
        const isDoubleBlink = Math.random() > 0.85;
  ...
  useEffect(() => {
    if (isTyping) {
      ...
      }, 1200); // Look at each other for 1.2 seconds
      return () => clearTimeout(timer);
    } else {
      // Occasional random glances at each other
      const glanceInterval = setInterval(() => {
        if (Math.random() > 0.92) {
          setIsLookingAtEachOther(true);
          setTimeout(() => setIsLookingAtEachOther(false), 600);
        }
      }, 4000);
  ...
  useEffect(() => {
    if (password.length > 0 && showPassword) {
      const schedulePeek = () => {
        ...
        }, Math.random() * 2000 + 1500); // More frequent peeking
        return peekInterval;
      };
